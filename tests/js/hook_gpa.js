...
hook.OnExit = function(api, args, retval) {
    ...
    if (last) {
        ...
        log("[hook] GetProcAddress returned: 0x" + last.retval.toString(16));
    }
};

hook.install("kernel32", "GetProcAddress");
log("[hook] GetProcAddress hook installed, args.length=" + hook.args.length);
