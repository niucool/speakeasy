...
pub const X86_EXPORTED_FUNCTION: &[u8] =
    b"\x8b\xff\x55\x8b\xec\xb8\x00\x00\x00\x00\x8b\xe5\x5d\xc3\xcc\xcc\xcc";
pub const X64_EXPORTED_FUNCTION: &[u8] =
    b"\x48\x89\xff\x90\x48\xc7\xc0\x00\x00\x00\x00\xc3\xcc\xcc\xcc\xcc";
...
pub fn normalize_dll_name(name: &str) -> String {
    ...
    if lower.starts_with("api-ms-win-crt")
        ...
        || lower.starts_with("ucrtbased")
        || lower.starts_with("ucrtbase")
        || lower.starts_with("msvcr")
...
impl JitPeFile {
    ...
    fn init_text_section(&mut self, names: &[String]) -> Vec<(usize, String)> {
        ...
        for (i, name) in names.iter().enumerate() {
            ...
            let mut func = if self.arch == "x86" {
                X86_EXPORTED_FUNCTION.to_vec()
            } else {
                X64_EXPORTED_FUNCTION.to_vec()
            };
