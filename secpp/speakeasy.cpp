...
Speakeasy::Speakeasy(const speakeasy::SpeakeasyConfig& cfg, 
    ...
    : config(cfg), emu_(nullptr), argv_(argv), exit_event_(exit_event), 
      debug_(debug) {
...
void Speakeasy::_init_emulator(const std::string& path, const std::vector<uint8_t>& data, bool is_raw_code) {
    ...
    if (!is_raw_code) {
        ...
        if (pe) {
            try {
                ...
                if (fh.Machine == 0x8664) {  // IMAGE_FILE_MACHINE_AMD64
                    pe_arch = speakeasy::arch::ARCH_AMD64;
                } else if (fh.Machine == 0x14c) {  // IMAGE_FILE_MACHINE_I386
                    pe_arch = speakeasy::arch::ARCH_X86;
                } else {
        ...
        if (is_driver) {
            ...
            emu_ = new speakeasy::WinKernelEmulator(config, argv_, debug_, exit_event_);
        } else {
            //  User-mode emulator (Python: Win32Emulator) 
            emu_ = new Win32Emulator(config, argv_, debug_, exit_event_);
        }
    } else {
    ...
    if (pe_arch == speakeasy::arch::ARCH_AMD64) {
        emu_->set_arch(speakeasy::arch::ARCH_AMD64);
    }
}
...
void Speakeasy::_init_hooks() {
    ...
    while (!instruction_hooks.empty()) {
        ...
        emu_->add_instruction_hook(cb, begin, end, {}, (BinaryEmulator*)this, insn);
    }
}
...
std::shared_ptr<speakeasy::RuntimeModule> Speakeasy::load_module(const std::string& path, const std::vector<uint8_t>& data) {
    ...
    return emu_->load_module(path, data);
}
...
uint64_t Speakeasy::load_shellcode(const std::string& fpath, const std::string& arch, 
    ...
    return emu_->load_shellcode(fpath, arch, data);
}

void Speakeasy::run_shellcode(uint64_t sc_addr, size_t stack_commit, size_t offset) {
    _init_hooks();
    emu_->run_shellcode(sc_addr, stack_commit, offset);
}

speakeasy::Report Speakeasy::get_report() { return emu_->get_report(); }

std::string Speakeasy::get_json_report() { return emu_->get_report().to_json_string(); }
...
std::shared_ptr<ApiHook> Speakeasy::add_api_hook(ApiCallback cb, const std::string& module,
    ...
    return emu_->add_api_hook(cb, module, api_name, argc, conv, (BinaryEmulator*)this);
}

void Speakeasy::resume(uint64_t addr, int count) { emu_->resume(addr, count); }

void Speakeasy::stop() { emu_->stop(); }

void Speakeasy::shutdown() { if (emu_) { delete emu_; emu_ = nullptr; } }
...
std::shared_ptr<DynCodeHook> Speakeasy::add_dyn_code_hook(DynCodeCallback cb, const std::map<std::string, std::string>& ctx) {
    ...
    return emu_->add_dyn_code_hook(cb, {});
}
...
std::shared_ptr<ReadMemHook> Speakeasy::add_mem_read_hook(MemAccessCallback cb, uint64_t begin, uint64_t end) {
    ...
    return emu_->add_mem_read_hook(cb, begin, end, emu_);
}
...
std::shared_ptr<WriteMemHook> Speakeasy::add_mem_write_hook(MemAccessCallback cb, uint64_t begin, uint64_t end) {
    ...
    return emu_->add_mem_write_hook(cb, begin, end, emu_);
}
...
std::shared_ptr<InvalidInstructionHook> Speakeasy::add_invalid_instruction_hook(InsnCallback cb, const std::vector<void*>& ctx) {
    ...
    return emu_->add_invalid_instruction_hook(cb, {}, emu_);
}
...
std::vector<std::shared_ptr<speakeasy::RuntimeModule>> Speakeasy::get_user_modules() {
    if (!emu_) return {};
    auto* win32 = dynamic_cast<Win32Emulator*>(emu_);
...
std::string Speakeasy::read_mem_string(uint64_t address, int width, size_t max_chars) {
    return emu_->read_mem_string(address, width, max_chars);
}
...
uint64_t Speakeasy::get_ret_address() { return emu_->get_ret_address(); }
void Speakeasy::set_ret_address(uint64_t addr) { emu_->set_ret_address(addr); }
void Speakeasy::push_stack(uint64_t val) { emu_->push_stack(val); }
uint64_t Speakeasy::pop_stack() { return emu_->pop_stack(); }
uint64_t Speakeasy::get_stack_ptr() { return emu_->get_stack_ptr(); }
void Speakeasy::set_stack_ptr(uint64_t addr) { emu_->set_stack_ptr(addr); }
uint64_t Speakeasy::get_pc() { return emu_->get_pc(); }
void Speakeasy::set_pc(uint64_t addr) { emu_->set_pc(addr); }
