...
// 
uint64_t Sfc_os::SfcIsFileProtected(void* e, std::vector<uint64_t>& a, void* ctx) {
    uint64_t hRpc = a[0];
...
// 
uint64_t Sfc_os::SfcTerminateWatcherThread(void* e, std::vector<uint64_t>& a, void* ctx) {
    (void)e; (void)a;
    return SFC_SUCCESS;
}
