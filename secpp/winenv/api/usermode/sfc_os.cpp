...
Sfc_os::Sfc_os(void* emu) : ApiHandler(emu) {
    INIT_API_TABLE(Sfc_os)
        REGX(Sfc_os, SfcTerminateWatcherThread, 0, 2)
...
// 
uint64_t Sfc_os::SfcIsFileProtected(void* e, ArgList& a, void* ctx) {
    uint64_t hRpc = a[0];
...
// 
uint64_t Sfc_os::SfcTerminateWatcherThread(void* e, ArgList& a, void* ctx) {
    (void)e; (void)a;
    return SFC_SUCCESS;
}
