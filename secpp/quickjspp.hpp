...
// Compatibility layer for quickjs-ng
typedef bool JS_BOOL;

inline thread_local JSRuntime * g_last_runtime = nullptr;

inline bool JS_IsArray_compat(JSContext *ctx, JSValueConst val) {
    (void)ctx;
    return JS_IsArray(val);
}
#define JS_IsArray JS_IsArray_compat

inline bool JS_IsError_compat(JSContext *ctx, JSValueConst val) {
    (void)ctx;
    return JS_IsError(val);
}
#define JS_IsError JS_IsError_compat

inline JSValue JS_ParseJSON2_compat(JSContext *ctx, const char *buf, size_t buf_len, const char *filename, int flags) {
    (void)flags;
    return JS_ParseJSON(ctx, buf, buf_len, filename);
}
#define JS_ParseJSON2 JS_ParseJSON2_compat

inline void JS_SetHostUnhandledPromiseRejectionTracker_compat(JSRuntime *rt, JSHostPromiseRejectionTracker *cb, void *opaque) {
    JS_SetHostPromiseRejectionTracker(rt, cb, opaque);
}
#define JS_SetHostUnhandledPromiseRejectionTracker JS_SetHostUnhandledPromiseRejectionTracker_compat

inline JSClassID JS_NewClassID_compat(JSClassID *pclass_id) {
    return (JS_NewClassID)(g_last_runtime, pclass_id);
}
#define JS_NewClassID JS_NewClassID_compat
...
public:
    ...
    ~Runtime()
    {
        if (g_last_runtime == rt) {
            g_last_runtime = nullptr;
        }
        JS_FreeRuntime(rt);
    }
