...
// quickjs-ng v0.14.0 compatibility shims
#ifndef JS_BOOL
#define JS_BOOL bool
#endif
#ifndef JS_ParseJSON2
#define JS_ParseJSON2(ctx, buf, len, fname, flags) JS_ParseJSON(ctx, buf, len, fname)
#endif
#ifndef JS_SetHostUnhandledPromiseRejectionTracker
...
struct js_traits<std::shared_ptr<T>>
{
    ...
    ensureCanCastToBase() {
        ...
        if(js_traits<std::shared_ptr<T>>::QJSClassId == 0) {
            // quickjs-ng requires a runtime; we don't have one here.
            // This path is only hit if register_class() wasn't called first,
            // which shouldn't happen in normal usage.
            static JSRuntime* fallback_rt = nullptr;
            JS_NewClassID(fallback_rt, &js_traits<std::shared_ptr<T>>::QJSClassId);
        }
