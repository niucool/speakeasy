...
// (types provided via scylla_types.h)  // was: #include <Psapi.h>
#include "PeParser.h"
...
{
    // Speakeasy mode: no real process handle needed — just track the PID
    if (SpeakeasyAccess::emulator()) {
        if (hProcess) return false;
        hProcess = (HANDLE)(DWORD_PTR)0xEA5E; // sentinel: Speakeasy mode
        return true;
    }
...
bool ProcessAccessHelp::readMemoryPartlyFromProcess(DWORD_PTR address, SIZE_T size, LPVOID dataBuffer)
{
    ...
    if (SpeakeasyAccess::emulator())
        return SpeakeasyAccess::readMemoryPartly(address, size, dataBuffer);
...
bool ProcessAccessHelp::writeMemoryToProcess(DWORD_PTR address, SIZE_T size, LPVOID dataBuffer)
{
    ...
    {
#ifdef DEBUG_COMMENTS
        Scylla::debugLog.log(L"writeMemoryToProcess :: hProcess == NULL");
#endif
        return false;
    }

    if (SpeakeasyAccess::emulator())
        return SpeakeasyAccess::writeMemory(address, size, dataBuffer);
...
bool ProcessAccessHelp::readMemoryFromProcess(DWORD_PTR address, SIZE_T size, LPVOID dataBuffer)
{
    ...
    // Speakeasy mode: read directly from emulated memory.
    if (SpeakeasyAccess::emulator())
        return SpeakeasyAccess::readMemory(address, size, dataBuffer);
...
{
    if (SpeakeasyAccess::emulator())
        return SpeakeasyAccess::getModules(moduleList);
...
{
    if (SpeakeasyAccess::emulator())
        return SpeakeasyAccess::getMemoryRegion(address, memoryRegionBase, memoryRegionSize);
...
SIZE_T ProcessAccessHelp::getSizeOfImageProcess(HANDLE processHandle, DWORD_PTR moduleBase)
{
    ...
    if (SpeakeasyAccess::emulator())
        return SpeakeasyAccess::getSizeOfImage(moduleBase);
